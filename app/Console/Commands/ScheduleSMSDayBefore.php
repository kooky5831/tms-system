...
class ScheduleSMSDayBefore extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:courseremindersmsdaybefore';
     ...
     * @var string
     */
    protected $description = 'Command for sending Course Reminder SMS One Day Before';
    ...
    public function handle()
    {
        ...
        $courseMains = $courseTriggersService->getAllCourseTriggersForSMS(2);
        // dd($courseMains);
        $weekBefore = Carbon::now()->addDays(1)->format('Y-m-d');
        // dd($weekBefore);
        ...
        foreach ($courseMains as $courseMain) {
            ...
            $courseRuns = Course::where('course_main_id', $courseMain->course_main_id)
                            ->whereDate('course_start_date', $weekBefore)->with(['courseActiveEnrolmentsWithStudent'])->get();
            // dd($courseRuns);
