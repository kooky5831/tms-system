        ...
        $(document).ready(function() {
            $(document).on('click', '#remarks_selection_btn', function(e) {
                ...
                $.ajax({
                    ...
                    url: '{{ route('admin.assessments.exam-settings.remark_student',  $getStudentEnroll->id) }}',
                    data: $('form.remarkStudent').serialize(),
                    type: "POST",
                    success: function(res, data) {
                        if( res.status ) {
                            showToast(res.msg, 1);
                            window.location.replace('{{ route('admin.assessments.exam-settings.review_studetn_exam_list_data', $getStudentEnroll->course_id) }}');
                        }
                        else{
                            showToast(res.msg, 0);
                        }
