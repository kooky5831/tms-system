...
<style>
    ...
    .student-name{ color: #E35F4A; }
    .save-assessment, .save-assessment:focus, .save-assessment:hover { background-color: #23365F !important; }
    .save-assessment:active { background-color: #3b5283 !important; }
</style>
@endpush
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="page-title-box">
                ...
                <h4 class="page-title">Grade Assessment: &nbsp;<span class="student-name"> {{ $studentData->student->name }} </span> </h4>
            </div><!--end page-title-box-->
        </div><!--end col-->
    </div>
    <div class="row">
        <div class="col-12">
            <button id="collapseAll" class="btn btn-primary mb-3 float-lg-right" style="width: 200px"> Expand All </button>
        </div>
                    ...
                    <li class="list-group-item mb-3">
                        <div class="collapse-module">
                            <div class="exam-collaps">
                                ...
                                <button class="btn btn-success btn-dark btn-module que_{{$review->id}} collapsed button-icon" type="button" data-toggle="collapse" data-target="#que_{{$review->id}}" aria-expanded="false" aria-controls="collapseExample" onclick="collapseToggle({{$review->id}})"><i class="fas fa-plus"></i></button>
                            </div>
                        </div>
                ...
                <div class="row mt-2 recovery-items">
                    <div class="col-12 recovery-item">
                        <div class="recovery-item-inner">
                            ...
                            <div class="all-check">
                                <button class="btn btn-info save-assessment" id="save_assessment">Save Assessment Result</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script type="text/javascript">
            ...
            $(document).ready(function() {
                $('#collapseAll').click(function(){
                    ...
                    var $accordionHeaders = $(".exam-collaps button");
                    var $accordionCollapses = $(".collapse");

                    var isCollapsed = $accordionHeaders.toArray().some(function(header) {
                        return !$(header).hasClass("collapsed");
                    });

                    if (isCollapsed) {
                        $button.html("Expand All");
                        $accordionCollapses.each(function() {
                            $(this).removeClass("show");
                            $('.button-icon').find('i').removeClass('fas fa-minus');
                        });
                        $accordionHeaders.each(function() {
                            $(this).addClass("collapsed");
                            $('.button-icon').find('i').addClass('fas fa-plus');
                        });
                    } else {
                        $button.html("Collapse All");
                        $accordionHeaders.each(function() {
                            $(this).removeClass("collapsed");
                            $('.button-icon').find('i').removeClass('fas fa-plus');
                        });
                        $accordionCollapses.each(function() {
                            $(this).addClass("show");
                            $('.button-icon').find('i').addClass('fas fa-minus');
                        });
                    }
                });          
            });
        ...
        class MyUploadAdapter {
            ...
            _initRequest() {
                ...
                xhr.open( 'POST', '{{route('trainer.exam-settings.ck-image-upload')}}', true );
        ...
        $("#save_assessment").on('click', function(){
            ...
            $.ajax({
                ...
                url: '{{route('trainer.exam-settings.assessment.exam.reviews')}}',
                type: "POST",
                dataType: "JSON",
                data: {
                ...
                success: function(res) {
                    ...
                    if(res.success){
                        window.location.replace('{{ route('trainer.exam-settings.view_trainees',  $studentData->course_id) }}');
                    }
                }
            }); // end ajax
        });
