...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4>Questions of Course Main:
                        {{-- @foreach ($allTrainerCourseRuns as $trainerCourse)
                        @php $count = 1 @endphp
                        @foreach ($getCourseMain as $courseMainData)
                                @if($courseMainData->coursemain_id == $trainerCourse->course_main_id)
                                    {{ $count .". ". $trainerCourse->courseMain->name }} -
                                @endif
                                @php $count++ @endphp
                            @endforeach
                        @endforeach --}}
                    ...
                    <div class="col-12 question-wrapper disabled-content">
                        <form method="POST" action="{{route('trainer.dashboard.add-questions', $allCourseRunQuestion->id)}}" id="question_form" enctype="multipart/form-data">
                            @csrf
...
<script type="text/javascript">
    ...
    function removeImage(imageId){
        $.ajax({
            ...
            url: '{{ route('trainer.exam-settings.delete-image') }}',
            type: "POST",
            dataType: "JSON",
            data: {
                id: imageId
