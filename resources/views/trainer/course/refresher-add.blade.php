...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <form action="{{ route('trainer.refreshers.add', $data->id) }}" id="courserefresher_add" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="student_id" id="student_id" class="form-control select2" required placeholder="Select Student">
                                        ...
                                        <option value="{{ $student->id }}" {{ old('student_id') == $student->id ? 'selected' : '' }}>{{ $student->name }} - {{ $student->nric }}</option>
                                        @endforeach
                                    </select>
                                    @error('student_id')
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="status" id="status" required  class="form-control select2">
                                        ...
                                        <option value="{{ $key }}" {{ old('status') == $key ? 'selected' : '' }}>{{ $statusText }}</option>
                                        @endforeach
                                    </select>
                    ...
                    <div class="card-footer m-0 clearfix">
                        ...
                        <a href="{{ route('trainer.course.courserunview', $data->id) }}" class="btn btn-danger">Cancel</a>
                    </div>
                </form>
            </div><!--end card-->
        </div> <!--end col-->
